...
ordered_layout <- data.frame(
  ...
  `# Fallos` = c("$d_0$", "$d_1$", "$d_2$", "$\\cdots$", "$d_k$"),
...
grupo2 <- data.frame(
  ...
  `d_f` = c(0, 2, 2, 1, 2, 2, 4, 2, 2, 1, 1, 1, 1, 1),
  `q_f` = rep(0, 14)
)
...
kable(df_g2[, c("t", "nf", "mf", "qf", "\\hat{S}(t_{(f)})")],
      col.names = c("$t_{(f)}$", "$n_f$", "$d_f$", "$q_f$", "$\\hat{S}(t_{(f)})$"),
